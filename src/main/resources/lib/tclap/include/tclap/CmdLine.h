...
public:
	 ...
	 */
	CmdLine(
	    const std::string& message, const char delimiter = ' ', const std::string& version = "none",
...
inline CmdLine::CmdLine(const std::string& m, char delim, const std::string& v, bool help)
    : _progName("not_set_yet"), _message(m), _version(v), _numRequired(0), _delimiter(delim), _handleExceptions(true),
      _userSetOutput(false), _helpAndVersion(help)
{
	_constructor();
}
