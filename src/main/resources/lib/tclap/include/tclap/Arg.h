...
inline Arg::Arg(
    ...
    : _flag(flag), _name(name), _description(desc), _required(req), _requireLabel("required"), _valueRequired(valreq),
      _alreadySet(false), _visitor(v), _ignoreable(true), _xorSet(false), _acceptsMultipleValues(false)
{
	if (_flag.length() > 1)
	...
	if (_name != ignoreNameString() &&
		...
		throw(
			...
			"Argument flag cannot be either '" + Arg::flagStartString() + "' or '" +
	...
	if ((_name.substr(0, Arg::flagStartString().length()) == Arg::flagStartString()) ||
		...
		throw(
		    SpecificationException(
			"Argument name begin with either '" + Arg::flagStartString() + "' or '" +
